        ...
        try:
            if result["data"] and result["data"]["login"] and result["data"]["login"]["token"]:
                if result["data"]["login"]["long"]==True:
                    response.set_cookie('auth',result["data"]["login"]["token"], domain="https://app.deform.ujjawal.co/",httponly=True, expires=(datetime.datetime.now()+datetime.timedelta(days=7)))
                else:
                    response.set_cookie('auth',result["data"]["login"]["token"], domain="https://app.deform.ujjawal.co/", httponly=True)
        except KeyError:
            pass
