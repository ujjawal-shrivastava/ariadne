        ...
        try:
            if result["data"] and result["data"]["login"] and result["data"]["login"]["token"]:
                if result["data"]["login"]["long"]==True:
                    response.set_cookie('auth',result["data"]["login"]["token"],httponly=True,secure=True, domain="deform.ujjawal.co", expires=(datetime.datetime.now()+datetime.timedelta(days=7)))
                else:
                    response.set_cookie('auth',result["data"]["login"]["token"],domain="deform.ujjawal.co", httponly=True,secure=True)
        except KeyError:
            pass
